...
public:
    map() : hashTable(cds_lfht_new(1, 1, 0, CDS_LFHT_AUTO_RESIZE | CDS_LFHT_ACCOUNTING, nullptr)) {
    ...
    void clear() {
        ...
        cds_lfht_for_each(hashTable, &it, node) {
            if (cds_lfht_del(hashTable, node) == 0) {
                defer_rcu(&caa_container_of(node, map::handle, node)->head, erase_node);
            }
        }
    }

    const std::optional<TValue> find(const TKey& key) const {

        cds_lfht_iter it;
        cds_lfht_lookup(hashTable, std::hash<TKey>()(key), equal_to, &key, &it);

        cds_lfht_node* node = cds_lfht_iter_get_node(&it);
        if (node == nullptr) return find_result<TValue>();

        return find_result<TValue>(static_cast<map::item*>(caa_container_of(node, map::handle, node))->value);
    }
    ...
    TValue& operator[](const TKey& key) const {
        ...
        cds_lfht_node* node = cds_lfht_iter_get_node(&it);
        if (node == nullptr) throw Exception("key not found");

        return static_cast<map::item*>(caa_container_of(node, map::handle, node))->value;
    }

    template<typename... TArgs>
    const std::pair<TValue&, bool> emplace(const TKey& key, TArgs&& ... args) {

        std::unique_ptr<map::item> item(new map::item(key, std::forward<TArgs>(args)...));
    ...
    bool update_if_not_equal(const TKey& key, TValue2&& value) {
        ...
        if (findNode != nullptr &&
            static_cast<map::item*>(caa_container_of(findNode, map::handle, node))->value == value)
            return false;

        map::item* item{new map::item(key, std::forward<TValue2>(value))};
        if (item == nullptr) throw Exception();

        cds_lfht_node* node = cds_lfht_add_replace(hashTable, std::hash<TKey>()(key), equal_to, &key, &item->node);
        if (node == nullptr) return true;

        call_rcu(&caa_container_of(node, map::handle, node)->head, erase_node);
        return true;
    }

    template<typename... TArgs>
    add_or_replace_result<TValue> add_or_replace(const TKey& key, TArgs&& ... args) {

        map::item* item{new map::item(key, std::forward<TArgs>(args)...)};
        if (item == nullptr) throw Exception();
        ...
        return add_or_replace_result<TValue>(item->value,
                                             static_cast<map::item*>(caa_container_of(node, map::handle, node))->value,
                                             true);
    }
    ...
    void for_each(TFunctor&& functor) const {
        ...
        cds_lfht_node* node;
        cds_lfht_for_each(hashTable, &it, node) {
            map::item* item = static_cast<map::item*>(caa_container_of(node, map::handle, node));
...
private:
    inline static int equal_to(cds_lfht_node* node, const void* key) {
        return *(TKey*) key == static_cast<map::item*>(caa_container_of(node, map::handle, node))->key;
    }

    inline static void erase_node(rcu_head* head) {
        delete static_cast<map::item*>(caa_container_of(head, map::handle, head));
    }
};
