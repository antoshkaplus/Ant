...
class Enqueue {
    ...
    ant::parallel::rcu::queue<TValue>& queue;
...
public:
    ...
    Enqueue(ant::parallel::rcu::queue<TValue>& queue, int count, TGenerator generator = []() {return TValue();})
...
public:
    Dequeue(ant::parallel::rcu::queue<TValue>& queue)
...
TEST(RcuQueue, Enqueue_1_Dequeue_1_Deleted) {
    std::atomic_int deleted_counter = 0;

    rcu_register_thread();

    ant::parallel::rcu::queue<ItemCounter> queue;
